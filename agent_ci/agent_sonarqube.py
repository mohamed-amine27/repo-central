...
def build_sonar_graph(tools: dict, llm: ChatSambaNova):
    ...
    # Noms des nœuds préfixés "step_" pour éviter le conflit
    # avec les clés de SonarState (issues, measures, fix_plan…)
    graph.add_node("step_scan",     _scan)
    graph.add_node("step_gate",     _gate)
    graph.add_node("step_issues",   _issues)
    graph.add_node("step_enrich",   _enrich)
    graph.add_node("step_fix",      _fix)
    graph.add_node("step_measures", _measures)
    graph.add_node("step_eval",     _eval)
    graph.add_node("step_report",   _report)

    graph.set_entry_point("step_scan")
    graph.add_edge("step_scan",     "step_gate")
    graph.add_edge("step_gate",     "step_issues")
    graph.add_edge("step_issues",   "step_enrich")
    graph.add_edge("step_enrich",   "step_fix")
    graph.add_edge("step_fix",      "step_measures")
    graph.add_edge("step_measures", "step_eval")
    graph.add_edge("step_eval",     "step_report")
